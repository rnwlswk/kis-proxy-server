...
// =========================
// 종목명으로 검색 - 네이버 금융 자동완성(비공식 API)을 이용. 정식 문서화된 API가 아니라서
// 응답 필드 구조가 예고 없이 바뀔 수 있음 - 방어적으로 여러 형태를 시도해서 파싱함
// =========================
app.get("/api/search-stock", async (req, res) => {
    try {
        ...
        console.error("[종목 검색 오류]", err.response?.data || err.message);
        res.status(500).json({ success: false, error: "종목 검색 실패" });
    }
});
